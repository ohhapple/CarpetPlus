...
import com.ohhapple.carpetplus.settings.PLUSRuleCategory;
...
public class NetworkUtil {
    ...
    public static final Set<String> PLUS_NETWORK_RULE_NAMES = new LinkedHashSet<>();
    ...
    public static void broadcastDataPack(PLUS_CustomPayload payload, SendMode sendMode) {
        MinecraftServerUtil.getOnlinePlayers().forEach(player -> sendS2CPacket(player, payload, sendMode));
    }

    public static void sendS2CPacket(ServerPlayer player, PLUS_CustomPayload payload, SendMode sendMode) {
    ...
    public static void sendC2SPacket(LocalPlayer player, PLUS_CustomPayload payload, SendMode sendMode) {
        boolean shouldSend = switch (sendMode) {
            case FORCE -> true;
    ...
    public static Set<String> getLocalSupportedPackets() {
        return new HashSet<>(PLUS_PayloadManager.PAYLOAD_REGISTRY.keySet());
    }
    ...
    public static void collectPLUSNetworkRuleNames() {
        for (Field field : CarpetPlusSettings.class.getDeclaredFields()) {
            if (field.isAnnotationPresent(Rule.class)) {
                Rule ruleAnnotation = field.getAnnotation(Rule.class);
                for (String category : ruleAnnotation.categories()) {
                    if (category.equals(PLUSRuleCategory.PLUS_NETWORK)) {
                        PLUS_NETWORK_RULE_NAMES.add(field.getName());
                        break;
                    }
                }
            }
