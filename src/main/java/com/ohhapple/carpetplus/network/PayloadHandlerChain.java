...
public class PayloadHandlerChain {
    private final Map<Class<? extends PLUS_CustomPayload>, CovariantHandlerList<?>> PAYLOAD_HANDLERS = new ConcurrentHashMap<>();

    public <T extends PLUS_CustomPayload> void put(Class<T> payloadClass, Consumer<T> handler) {
        CovariantHandlerList<T> list = getOrCreate(payloadClass);
        list.addHandler(handler);
    }

    private <T extends PLUS_CustomPayload> CovariantHandlerList<T> getOrCreate(Class<T> type) {
    ...
    private <T extends PLUS_CustomPayload> CovariantHandlerList<T> castList(CovariantHandlerList<?> list, Class<T> type) {
    ...
    public boolean handle(PLUS_CustomPayload payload) {
        if (payload == null) {
            return false;
        }
    ...
    private static class CovariantHandlerList<T extends PLUS_CustomPayload> {
        ...
        boolean handle(PLUS_CustomPayload payload) {
        ...
        boolean matchesType(Class<? extends PLUS_CustomPayload> otherType) {
            return type.equals(otherType);
        }

        <U extends PLUS_CustomPayload> CovariantHandlerList<U> asType(Class<U> targetType) {
        ...
        private <U extends PLUS_CustomPayload> Consumer<U> createBridgeConsumer(Consumer<T> originalHandler) {
            return u -> {
