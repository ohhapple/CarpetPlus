...
public class HandShakeC2SPayload extends PLUS_CustomPayload {
    private static final String ID = PLUS_PayloadManager.PacketId.HANDSHAKE_C2S.getId();
    ...
    public void handle() {
        NetworkUtil.executeOnServerThread(() -> {
            ...
            if (this.modVersion.equals(CarpetPlus.getVersion())) {
                ohhappleinit.LOGGER.info("{} joined with matched carpetplus v{}", playerName, this.modVersion);
            } else {
                ohhappleinit.LOGGER.info("{} joined with mismatched carpetplus version (client: v{}, server: v{})", playerName, this.modVersion, CarpetPlus.getVersion());
            }
            ...
            if (player != null) {
                ohhappleinit.getInstance().sendS2CPacketOnHandShake(player);
            } else {
                ohhappleinit.LOGGER.warn("Could not find player entity for UUID: {}", this.playerUuid);
            }
        });
    }
